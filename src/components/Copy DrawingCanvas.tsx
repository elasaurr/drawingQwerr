...
export default function DrawingCanvas() {
	...
	useEffect(() => {
		...
		handleAddLayer();
		// setActiveLayerId(String(layers[0].id));
	}, []);
		...
		[...layersToRender].reverse().forEach((layer) => {
			...
			img.onload = () => {
				if (layer.id === activeLayerId) {
					console.log("DrawingCanvas renderAllLayers activeLayerId", activeLayerId);
					console.log("DrawingCanvas renderAllLayers opacity", layer.opacity);
				}
				ctx.save();
	...
	useEffect(() => {
		...
		if (activeLayer?.imageData) {
			...
			console.log("DrawingCanvas useEffect activeLayer: ", JSON.stringify(activeLayer));
	...
	const handleAddLayer = () => {
		...
		const savedImageData = layerCanvas.toDataURL();
		const newLayerId = crypto.randomUUID();
		console.log("DrawingCanvas handleAddLayer newLayerId", newLayerId);
		...
		setActiveLayerId(String(newLayerId));
		console.log("DrawingCanvas handleAddLayer activeLayerId", activeLayerId);
	};
							...
							<TabsContent value="tools" className="p-4 space-y-6 mt-0">
								...
								<div>
									...
									<div className="flex gap-2">
										...
										/>
										<Input type="text" value={color} onChange={(e) => setColor(e.target.value)} className="flex-2" />
									</div>
								</div>
				...
				{showLayerPanel && (
					<LayerPanel
						...
						onLayersChange={(newLayers) => {
							...
							renderAllLayers(newLayers);
							console.log(
								"LayerPanel onLayersChange newLayers",
								newLayers.map((l) => l.name)
							);
						}}
